...
- **Durations**: local files get their exact duration automatically, shortly after being added, by reading the file's metadata (no internet needed). YouTube videos are different — there's no way to learn a video's duration without either loading it or using YouTube's Data API (which needs an API key this app doesn't ask you to set up), so a YouTube video shows "—" until you've played it at least once, at which point its duration is captured and remembered from then on. The playlist total shown under its name only counts videos whose duration is currently known, and says how many are still unknown rather than silently under-reporting.
